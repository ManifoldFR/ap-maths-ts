...
\begin{enumerate}
\item Sachant que $\PP(A) = 1/3$ et $\PP_A(B) = 2/5$ et $\PP_{\bar A}(B)=1/4$.
\item Sachant que $\PP(A) = 2/3$, $\PP(A\cap B)=1/3$ et $\PP(\overline A\cap B) = 1/6$.
\end{enumerate}


\exercice
Soit $\mathbb U_n$ l'ensemble des nombres complexes $a$ tels que $a^n=1$, où $n\geq 2$. On montre que ses éléments sont de la forme
\[
e^{\frac{2ik\pi}{n}},\ 1\leq k\leq n.
\]
...
On prend deux éléments $w$ et $z$ de $\mathbb U_n$ au hasard; on note $M_w$ et $M_z$ les points du plan complexe dont ils sont les affixes; quelle est la valeur moyenne de l'angle $\angle(M_wOM_z)$ ?
